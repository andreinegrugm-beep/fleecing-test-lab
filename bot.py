...
QUESTIONS = [
    ("sample", "Какой образец тестируете?", ["SAFEUP V1-HS0.7", "SAFEUP V1.1-HS0.7", "Другой образец"]),
    ("name", "Введите ваше имя:", None),
    ("hair_type", "Тип волос?", ["Тонкие", "Нормальные", "Плотные", "Окрашенные", "Осветленные"]),
    ("application", "Как использовали средство?", ["Влажные волосы + фен", "Сухие волосы + фен", "Утюжок", "Плойка"]),
    ("spray", "Оцените распыление от 1 до 10:", [str(i) for i in range(1, 11)]),
    ("comb", "Легкость расчесывания от 1 до 10:", [str(i) for i in range(1, 11)]),
    ("smooth", "Гладкость волос от 1 до 10:", [str(i) for i in range(1, 11)]),
    ("shine", "Блеск волос от 1 до 10:", [str(i) for i in range(1, 11)]),
    ("volume", "Объем у корней от 1 до 10:", [str(i) for i in range(1, 11)]),
    ...
    ("comment", "Комментарий: что понравилось / не понравилось?", None),
]
